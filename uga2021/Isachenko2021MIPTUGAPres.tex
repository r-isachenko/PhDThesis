...
\begin{frame}{Dimensionality Reduction for Signal Decoding}
	The problem of investigation is a model selection in the case of high hidden correlations in the input and target spaces.
	...
	\begin{block}{Goal}
		Build model that adequately describes the input and target spaces with the observed multicorrelation in both spaces.
	\end{block}
	
	\begin{block}{Solution}
		We propose the dimensionality reduction methods which project the input and target variables into the hidden low-dimensional space. 
...
\begin{frame}{Concordance process in signal decoding task}
	...
	The latent spaces $\bbT$ и $\bbU$ are called \textbf{concordant}, if there is \textit{link function} $\mathbf{h}: \bbT \rightarrow \bbU$:
	\vspace{-0.3cm}
	\[
...
\begin{frame}{Latent space concordant function}
	\begin{statement}[Isachenko, 2017]
		Vectors $\bt_k$ и $\bu_k$ obtained by iterative procedure:
		\vspace{-0.2cm}
		\begin{align*}
	...
	\begin{theorem}[Isachenko, 2017]
		In the case of linear encoding function $\bphi_e (\bT) = \bT \bP^{\T}$, $\bpsi_e (\bU) = \bU \bQ^{\T}$ and concordance function $g(\bt, \bu) = \textnormal{cov}(\bt, \bu)$ the model parameters
		\[
...
\begin{frame}{Example of hidden space concordant projection}
	Input variables $\bx_i \sim \mathcal{N}(0, \mathbf{\Sigma})$. \\ 
	Target variables $\by_i$ linearly depend on $pc_2$ и does not depend on $pc_1$.
	\begin{figure}[h]
	\centering
...
\begin{frame}{Superposition of signal decoding models}
	...
	\begin{statement}[isachenko, 2021]
		If decoding model is an additive superposition of linear models:
		\vspace{-0.3cm}
		\[
	...
	\begin{theorem}[Isachenko, 2021]
		If $\text{span}(\bX_1) \neq \text{span}(\bX_2)$, then the error of additive superpositionof linear decoding models is less or equal of each of the model:
		\begin{align*}
...
\begin{frame}{Nonlinear methods of hidden space concordance}
	Let consider the case where encoding and decoding functions are deep neural networks:
	\vspace{-0.1cm}
	\begin{align*}
	...
	\end{block}	
	\vspace{-0.3cm}
		Gradient of concordance function $g(\bt, \bu) = \textnormal{corr}(\bt, \bu)$ is
		\[
...
\begin{frame}{Feature selection for signal decoding task}
	$\bX = [\bchi_1, \dots, \bchi_n] \in \bbR^{m \times n}$~-- input variable matrix; $\bY = [\bnu_1, \dots, \bnu_r] \in \bbR^{m \times r}$~-- target variable matrix. \\
...
\begin{frame}{Feature selection in signal decoding task}
	...
	\] 
	\end{block}
	SymImp penalizes correlated target vectors that are less explained by features.
	...
	\begin{block}{Assymetric Importance (AsymImp)}
		...
		\end{equation*}
		\vspace{-0.4cm} \\
		In the case of $b_j = \max\limits_{i=1, \dots n} [\bB]_{i, j}$ components of~$\bz_y$ in~$\text{Rel}(\bX, \bY)$ are non-negative.
	\end{block}
\end{frame}
%--------------------------------------------------------------------------------
\begin{frame}{Feature selection in signal decoding task}
...
\begin{frame}{Generalization of the proposed feature selection methods}
	\begin{theorem}[Isachenko, 2018]
		In the case of scalar target variable $r=1$, the proposed feature selection methods (SymImp, MinMax, MaxMin, AsymImp) coincide with the original task of minimizing the loss function $S(\bz, \bX, \bY)$.
	\end{theorem}
	\begin{table}
		\centering
		\footnotesize{
...
\begin{enumerate}
	...
	\vfill
	Nonlinear methods for hidden space concordance for complex structured target variable are proposed .
	\vfill
	We have proposed the number of forecasting models for heterogeneous sets of signals that are suitable for the task of modelling brain computer interfaces.
\end{enumerate}
\end{frame}
%--------------------------------------------------------------------------------
