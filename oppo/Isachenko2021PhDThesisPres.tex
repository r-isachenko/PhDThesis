...
\begin{frame}{Задача декодирования сигналов}
	...
	$\mathbf{f}_{\bx\by}: \bbR^{h_x \times m} \times \bbR^{h_y \times r} \rightarrow \bbR^{p \times r}$ -- модель декодирования.
\end{frame}
%--------------------------------------------------------------------------------
...
\begin{frame}{Согласование зависимостей в задаче декодирования}
	...
	\text{для любого } \by \in \bbY &\quad  \text{существует } \bu \in \bbU: \bpsi_{\by} \bigl(\bphi_{\by}(\by)\bigr) = \bpsi_{\by}(\bu) = \by.
	\end{align*}
...
\begin{frame}{Суперпозиция моделей декодирования сигналов}
	...
	\begin{rustheorem}[Исаченко, 2021]
		Ошибка аддитивной суперпозиции линейных моделей декодирования не превышает ошибки отдельной модели:
		\begin{align*}
...
\begin{frame}{Нелинейные методы согласования скрытого пространства}
	...
	\begin{align*}
		...
		\bX &= \bpsi_{\bx}(\bT) =  \bW_t^L \sigma(\dots \sigma(\bW_t^2 \sigma(\bT \bW_t^1)) \dots ) \\
		\bY &= \bpsi_{\by}(\bU) =  \bW_u^L \sigma(\dots \sigma(\bW_u^2 \sigma(\bU \bW_u^1)) \dots )
	\end{align*}
	\vspace{-0.4cm}
	\begin{block}{Согласование проекций}
		\vspace{-0.3cm}
...
\begin{frame}{Выбор признаков в задаче декодирования}
	...
	\end{rustheorem}
	Для получения выпуклой задачи применяется полуопределенная релаксация сдвига спектра.
...
\begin{frame}{Оптимизация нелинейных моделей с помощью квадратичного программирования}
	\begin{rustheorem}[Исаченко, 2018]
		...
		Тогда метод выбора признаков с помощью квадратичного программирования имеет параметры
		\[
			\bQ = \textnormal{Sim}(\bJ), \quad \mathbf{b} = \textnormal{Rel}(\bJ, \by - \mathbf{f}).
		\]
	\end{rustheorem}
	\begin{rustheorem}[Исаченко, 2018]
		Пусть модель имеет вид $f(\bx , \btheta) = \sigma(\bx^{\T} \btheta)$.  Тогда метод выбора признаков с помощью квадратичного программирования для функции ошибки логистической регрессии имеет параметры
		\[
			\bQ = \textnormal{Sim}(\bR^{1/2} \bX), \quad \mathbf{b} = \textnormal{Rel}(\bR^{1/2} \bX, \bR^{-1/2} \by - \mathbf{f}).
		\]
	\end{rustheorem}
\end{frame}
%--------------------------------------------------------------------------------
